...
def loss_push_apart(f1, f2, margin=2.0):
        """
        拉远两个特征向量
        f1, f2: [batch, dim]
        margin: 最小间隔
        ...
        # 惩罚过近的向量，让它们至少相距 margin
        loss = torch.mean(F.relu(margin - dist))
        return loss
...
class Base_Model(nn.Module):
    ...
    # def forward(self, inputs):
        ...
        loss_1 = loss_push_apart(b1,fusion_feature)
        loss_2 = loss_push_apart(b2,fusion_feature)
        loss_apart = self.w1*(loss_1+loss_2)

        logit = self.decision_branch(fusion_feature)

        return logit,loss_apart
